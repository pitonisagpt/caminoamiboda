...
class Reservation(Base):
    ...
    # Contract-specific: what vinilo/decoración/ploteo is authorized on the
    # vehicle, and by when it must be removed. Null/empty decoration_details
    # means clause SEXTA of ReservationContract states none was authorized.
    decoration_details: Mapped[Optional[str]] = mapped_column(Text(), nullable=True)
    decoration_removal_date: Mapped[Optional[date]] = mapped_column(Date(), nullable=True)
    ...
    @property
    def display_driver_whatsapp_username(self) -> Optional[str]:
        if self.owner_driver_id and self.owner_driver:
            return getattr(self.owner_driver, "whatsapp_username", None)
        if self.driver:
            return getattr(self.driver, "whatsapp_username", None)
        return None
