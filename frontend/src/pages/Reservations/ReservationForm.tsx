...
import VehicleQuickCreateModal from './VehicleQuickCreateModal';
import DriverQuickCreateModal from './DriverQuickCreateModal';
...
export default function ReservationForm() {
  ...
  const onSubmit = async (data: ReservationFormData) => {
    ...
    if (isEdit) {
      ...
      savedId = Number(id);
      gcalSynced = res.data.gcal_synced;
    } else {
      const res = await reservationsApi.create(payload);
      savedId = res.data.id;
      gcalSynced = res.data.gcal_synced;
    }
    navigate(`/reservas/${savedId}`, { state: { gcalSynced } });
  };
