  ...
  return (
    <div className="bg-white border border-brand-200 rounded-xl p-4 shadow-sm">
      <div className="flex items-start justify-between mb-2">
        <h3 className="font-semibold text-gray-900 text-sm">{t("aiLeadCapture.title")}</h3>
        <button onClick={onClose} className="text-gray-400 hover:text-gray-600 cursor-pointer">
          <X size={16} />
        </button>
      </div>
      <p className="text-xs text-gray-500 mb-3">
        {t("aiLeadCapture.subtitle")}
      </p>
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="absolute -left-[9999px] w-px h-px overflow-hidden" aria-hidden="true">
          <label htmlFor="hp_website_chat">{t("contacto.honeypotLabel")}</label>
          <input
            id="hp_website_chat"
            name="hp_website"
            type="text"
