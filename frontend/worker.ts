...
export default {
  async fetch(request: Request, env: Env, _ctx: ExecutionContext): Promise<Response> {
    try {
      ...
      // /sitemap.xml is generated by the backend (app/routers/seo.py), not
      // a static file in this repo — this is a pure static-assets deploy
      // (see wrangler.jsonc), so without this the SPA's own client-side
      // router would catch the request and render its 404 page instead
      // (confirmed live: caminoamiboda.com/sitemap.xml showed "Página no
      // encontrada"). robots.txt already pointed the *declared* sitemap
      // at api.caminoamiboda.com/sitemap.xml directly, which always
      // worked fine for Google/crawlers — but a human (or any tool)
      // trying the intuitive https://caminoamiboda.com/sitemap.xml got a
      // 404. Proxying it here makes both URLs serve the identical,
      // always-current sitemap with zero duplicated logic.
      if (pathname === "/sitemap.xml") {
        // Same 5s AbortController timeout convention as findVehicle()
        // below — if the backend is slow/down, fail fast into the outer
        // catch (falls through to the plain SPA/ASSETS response) instead
        // of holding the request open indefinitely.
        const controller = new AbortController();
        const timeoutId = setTimeout(() => controller.abort(), 5000);
        try {
          const upstream = await fetch(`${env.API_BASE_URL}/sitemap.xml`, { signal: controller.signal });
